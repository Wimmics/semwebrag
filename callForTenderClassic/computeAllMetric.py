...
import nltk
import pandas as pd
from callForTenderClassic.getAllQA import getAllQA
import json
import re
import time
...
if not os.path.exists('callForTenderClassic/evaluation3.csv'):
    with open('callForTenderClassic/evaluation3.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['nChunks', 'METEOR', 'BLEU', 'BERT', 'ROUGE','OVERLAP','OVERLAPE', 'TEMPS'])

    for i in range(11):
        with open('callForTenderClassic/evaluation3.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([i, 0, 0, 0, 0, 0, 0, 0])

def addToCSV(nChunks, meteor, bleu, bert, rouge, overlap, overlapE, temps_moyen):
    df = pd.read_csv('callForTenderClassic/evaluation3.csv')
    ...
    if mask.any():
        ...
        df.to_csv('callForTenderClassic/evaluation3.csv', index=False)
...
while(nChunk <= 10):
    ...
    for question in questions:
        ...
        try:
            ...
            result = subprocess.run([venv_python, '-m', 'callForTenderClassic.eval', str(nChunk), question, answer], 
                                      capture_output=True, 
                                      text=True, 
                                      check=True)
