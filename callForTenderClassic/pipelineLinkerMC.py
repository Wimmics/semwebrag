...
with open("callForTenderClassic/text.txt", "r", encoding="utf-8") as file:
    text = file.read()
...
def build_knowledge_graph_aligned_with_ontology(text,ontology_path, nlp, rdf_path, embeddings):
    ...
    DAO.save_index("callForTenderClassic/embeddings.index")
    


import re

def process_query(query_text, rdf_graph_path, embeddings_model=embeddings, output_file="callForTenderClassic/query_enrichie.txt", neighborChunks=0): #traite la query, trouve les entités pertinente dans le graphe et enrichit la query avec les chunks liés
    doc = nlp(text)
    tload = time.time()
    DAO = FaissDAO(384)
    DAO.load_index("callForTenderClassic/embeddings.index")
    ...
    t = time.time()
    wikidatautils.initialize_json_file("callForTenderClassic/logs.json")
    ...
    wikidatautils.add_to_json_file("callForTenderClassic/logs.json", entity_data)
...
# build_knowledge_graph_aligned_with_ontology(text, "callForTenderClassic/public-contracts.ttl", nlp, "callForTenderClassic/knowledge_graphNoWiki.ttl", embeddings)
