...
def get_random_question_response() : 
    ...
    with open("medical/questions_reponses.csv", "r", encoding="utf-8") as file:
...
def evaluate_metrics(nChunks,question, true_answer):
    ...
    try:
        result = subprocess.run([venv_python, '-m', 'medical.userPrompt', question, nChunks], 
                                 capture_output=True, 
                                 text=True, 
                                 check=True)  
    ...
    try:
        ...
        pa = subprocess.run([venv_python, '-m', 'medical.prompt2'], capture_output=True, text=True, check=True)
