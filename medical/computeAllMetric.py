...
if not os.path.exists('medical/evaluation3.csv'):
    with open('medical/evaluation3.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['nChunks', 'METEOR', 'BLEU', 'BERT', 'ROUGE','OVERLAP','OVERLAPE', 'TEMPS'])

    for i in range(11):
        with open('medical/evaluation3.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([i, 0, 0, 0, 0, 0, 0, 0])

def addToCSV(nChunks, meteor, bleu, bert, rouge, overlap, overlapE, temps_moyen):
    df = pd.read_csv('medical/evaluation3.csv')
    ...
    if mask.any():
        ...
        df.to_csv('medical/evaluation3.csv', index=False)
...
while(nChunk <= 10):
    ...
    for question in questions:
        ...
        try:
            ...
            result = subprocess.run([venv_python, '-m', 'medical.eval', str(nChunk), question, answer], 
                                      capture_output=True, 
                                      text=True, 
                                      check=True)
