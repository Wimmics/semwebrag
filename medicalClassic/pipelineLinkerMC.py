...
with open("medicalClassic/text.txt", "r", encoding="utf-8") as file:
    text = file.read()
...
def build_knowledge_graph_aligned_with_ontology(text,ontology_path, nlp, rdf_path, embeddings):
    ...
    DAO.save_index("medicalClassic/embeddings.index")
    


import re

def process_query(query_text, rdf_graph_path, embeddings_model=embeddings, output_file="medicalClassic/query_enrichie.txt", neighborChunks=0): #traite la query, trouve les entités pertinente dans le graphe et enrichit la query avec les chunks liés
    doc = nlp(text)
    tload = time.time()
    DAO = FaissDAO(384)
    DAO.load_index("medicalClassic/embeddings.index")
    ...
    t = time.time()
    wikidatautils.initialize_json_file("medicalClassic/logs.json")
    ...
    wikidatautils.add_to_json_file("medicalClassic/logs.json", entity_data)
...
# build_knowledge_graph_aligned_with_ontology(text, "medicalClassic/dev.fibo-quickstart.ttl", nlp, "financeClassic/knowledge_graphNoWiki.ttl", embeddings)
