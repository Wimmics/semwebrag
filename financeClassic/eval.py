...
def evaluate_metrics(nChunks, question, true_answer):
    ...
    try:
        result = subprocess.run([venv_python, '-m', 'financeClassic.userPrompt', question, nChunks], 
                                 capture_output=True, 
                                 text=True, 
                                 check=True)  
    ...
    try:
        ...
        pa = subprocess.run([venv_python, '-m', 'financeClassic.prompt2'], capture_output=True, text=True, check=True)
