...
if not os.path.exists('financeClassic/evaluation3.csv'):
    with open('financeClassic/evaluation3.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['nChunks', 'METEOR', 'BLEU', 'BERT', 'ROUGE','OVERLAP','OVERLAPE', 'TEMPS'])

    for i in range(11):
        with open('financeClassic/evaluation3.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([i, 0, 0, 0, 0, 0, 0, 0])

def addToCSV(nChunks, meteor, bleu, bert, rouge, overlap, overlapE, temps_moyen):
    df = pd.read_csv('financeClassic/evaluation3.csv')
    ...
    if mask.any():
        ...
        df.to_csv('financeClassic/evaluation3.csv', index=False)
