...
with open("callForTender/SOLIHAdata.txt", "r", encoding="utf-8") as file:
    text = file.read()
...
def build_knowledge_graph_aligned_with_ontology(text,ontology_path, nlp, rdf_path, embeddings):
    ...
    DAO.save_index("callForTender/embeddings.index")
    convert_wikidata_with_regex("callForTender/outputLinkerLinked.ttl", "callForTender/outputLinkerLinked.ttl")
    return g
...
def process_query(query_text, rdf_graph_path, embeddings_model=embeddings, output_file="callForTender/query_enrichie.txt", neighborChunks=0): #traite la query, trouve les entités pertinente dans le graphe et enrichit la query avec les chunks liés
    tload = time.time()
    DAO = FaissDAO(384)
    DAO.load_index("callForTender/embeddings.index")
    g = Graph()
    ...
    t = time.time()
    wikidatautils.initialize_json_file("callForTender/logs.json")
    ...
    for ent in query_entities:
        ...
        wikidatautils.add_to_json_file("callForTender/logs.json", entity_data)
...
# build_knowledge_graph_aligned_with_ontology(text, "callForTender/public-contracts.ttl", nlp, "callForTender/knowledge_graphNoWiki.ttl", embeddings)
...
# process_query("Que peux tu me dire sur l'ARTICLE 7 ? Quelles en sont les clauses ? ","callForTender/outputLinkerLinked.ttl", neighborChunks=4)
